...
/* According to my basic sql optimalization knowledge - this query seems optimized to me pretty well. 
		- WHERE is used rarely, INNER JOINS or LEFT JOINS with AND statement is used instead
        - OR statement is not used at all
        - columns seems to be selected wisely
*/
SELECT 
	RSEG.EBELN,
	RSEG.EBELP,
...
INNER JOIN "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_BSEG" AS BSEG ON 1=1
    ...
    AND BSEG.DMBTR*-1 >= 0
-- select specific columns instead of "*", if possible
